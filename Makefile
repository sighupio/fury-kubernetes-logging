...
test-apply:
	...
	vagrant ssh -c "docker run -ti -w /test_dir -v /workspace/sighup/fury-kubernetes-logging:/test_dir -v /workspace:/workspace -e LOCAL_DEV_ENV=true -e CLUSTER_HOST=localhost -e KUBECONFIG=/workspace/kubeconfig --network=host sighup/kind:v1.12.3-bats \"bats -p katalog/tests/tests.bats\"" $(FURY_VM_ID)
